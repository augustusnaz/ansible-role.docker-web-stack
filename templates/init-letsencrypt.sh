...
domains=({% for subdomain in app_subdomains %}{% if subdomain != '' %}{{ subdomain }}.{% endif %}{{ app_domain }} {% endfor %} {% if app_backend %}{{ app_backend }}.{{ app_domain }}{% endif %} {% if app_install_mailserver %}{{ mailserver_hostname }}.{{ app_domain }}{% endif %})
rsa_key_size=4096
...
# if [ -d "$data_path" ]; then
if [ -d "$data_path/letsencrypt/live/$domains" ]; then
